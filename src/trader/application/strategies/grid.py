...
class DynamicGridStrategy(BaseStrategy):
    ...
    def __init__(self, 
        ...
        self.sar_trailing_stop: Optional[Decimal] = None
    ...
    def _handle_breakout(self, direction: str, current_price: Decimal):
        ...
        # 即使没有持仓，也应该检查是否可以进入 SAR 模式
        # if not position or position.quantity == 0:
        #     # Nothing to stop, just reset grid
        #     self.cancel_and_reset_grid(current_price)
        #     return

        entry_price = position.entry_price if position else Decimal("0")
        side_str = str(position.side).split('.')[-1] if position else "NONE"
        ...
        # 即使没有持仓，如果价格突破了网格边界，也应该考虑进入 SAR 模式
        if not stop_triggered and (not position or position.quantity == 0):
            if direction == "UP" and current_price >= self.upper_limit:
                print(f"🛑 [STOP] Grid Range Broken (No Position)! Price {current_price} >= Upper Limit")
                stop_triggered = True
            elif direction == "DOWN" and current_price <= self.lower_limit:
                print(f"🛑 [STOP] Grid Range Broken (No Position)! Price {current_price} <= Lower Limit")
                stop_triggered = True
        ...
        if stop_triggered:
            ...
            if position and position.quantity > 0:
                ...
                print(f"⚡ [SAFETY] 2. No Position to Close.")
